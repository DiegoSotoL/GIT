...
router.get('/colegios/add', isLoggedIn, (req,res) =>{
   res.render('crud_admin/colegios/add');
});

router.post('/colegios/add', isLoggedIn, async (req,res) =>{
 ...
 router.get('/colegios', isLoggedIn, async (req,res) => {
   const colegios = await pool.query('SELECT * FROM colegios');
   res.render('crud_admin/colegios/list',{ colegios });
 });

 router.get('/colegios/delete/:id', isLoggedIn, async(req,res) => {
...
router.get('/colegios/edit/:id', isLoggedIn, async (req,res) => {
...
router.post('/colegios/edit/:id', isLoggedIn, async ( req , res ) => {
   const{id} = req.params;
   const {nombre,direccion,telefono,correo} = req.body;
   const nuevoColegio = {
      nombre,
